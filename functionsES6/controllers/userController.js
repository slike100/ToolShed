  ...
  try {
    db.collection("User")
      ...
      .then(() => {
        return res
          .status(200)
          .send("A new user was successfully created in the database.");
      });
  } catch (err) {
    ...
    db.collection("User")
      ...
      .then(async () => {
        ...
        return res
          .status(200)
          .send(
            "Sorry to see you go! Your account has been successfully deleted."
          );
      });
  } catch (err) {
    return res
      .status(500)
      .send(
        "Sorry, we were unable to delete your account. Please try again or contact support."
      );
  }
});
